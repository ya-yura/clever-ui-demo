 ...
 * Order: In Progress (1) -> Over-plan/Errors (2) -> Not Started (3) -> Completed (4)
 */
export function getLinePriority(line: ReceivingLine): number {
  ...
  // Over-plan: received more than planned - ERRORS (с ошибками)
  if (fact > plan) return 2;

  // Not started: nothing received yet (предстоит сделать)
  if (fact === 0) return 3;

  // Completed: received exactly as planned - LOWEST PRIORITY (уже сделано)
